...
infection_data <- merge(read.csv(file = "Data/Infection_data.csv", header = T, stringsAsFactors = F), recombined_construct_key, by = "recombined_construct") %>% arrange(date)
```

## Introducing LLP Int-iCasp9-Blast
```{r Introducing LLP Int-iCasp9-Blast}
llp_comparison1 <- read.csv(file = "Data/Int-iCasp9-Blast/Int-iCasp9-Blast_200317.csv", header = T, stringsAsFactors = F)
llp_comparison2 <- read.csv(file = "Data/Int-iCasp9-Blast/Int-iCasp9-Blast_200624.csv", header = T, stringsAsFactors = F)
llp_comparison3 <- read.csv(file = "Data/Int-iCasp9-Blast/Int-iCasp9-Blast_200702.csv", header = T, stringsAsFactors = F)
llp_comparison4 <- read.csv(file = "Data/Int-iCasp9-Blast/Int-iCasp9-Blast_200706.csv", header = T, stringsAsFactors = F)
...
staining_summary_frame$cell_label <- factor(staining_summary_frame$cell_label, levels = c("None","ACE2","ACE2(IRES-mCherry)","ACE2-T2A-mCherry","ACE2-mCherry"))
...
staining_summary_frame2$cell_label <- factor(staining_summary_frame2$cell_label, levels = c("None","ACE2","ACE2(IRES-mCherry)","ACE2-T2A-mCherry","ACE2-mCherry"))

Construct_mCherry_and_stain <- ggplot() + theme_bw() + scale_y_continuous(limits = c(0,100), expand = c(0,0.1)) +
  ...
  geom_point(data = staining_summary_frame2, aes(x = cell_label, y = mean_pct_red_also_grn), position = position_dodge(width = 0.4), size = 8, shape = 95)
Construct_mCherry_and_stain
ggsave(file = "Plots/Construct_mCherry_and_stain.pdf", Construct_mCherry_and_stain, height = 2.5, width = 1.5)
```
...
staining_initial_constructs$cell_label <- factor(staining_initial_constructs$cell_label, levels = c("None","ACE2(IRES-mCherry)","ACE2","ACE2-T2A-mCherry","ACE2-mCherry"))
staining_initial_constructs_summary$cell_label <- factor(staining_initial_constructs_summary$cell_label, levels = c("None","ACE2(IRES-mCherry)","ACE2","ACE2-T2A-mCherry","ACE2-mCherry"))
...
Initial_construct_staining <- ggplot() + theme_bw() + 
  ...
  geom_errorbar(data = staining_initial_constructs_summary, aes(x = cell_label, ymin = lower_ci, ymax = upper_ci, color = pseudovirus_inoc), alpha = 0.2, width = 0.2, position = position_dodge(width = 0.4)) +
...
ggsave(file = "Plots/Initial_construct_staining.pdf", Initial_construct_staining, height = 2, width = 4)
...
Initial_construct_staining_small <- ggplot() + theme_bw() + 
  ...
  scale_y_log10() + scale_color_manual(values = c("black","orange")) +
  geom_errorbar(data = staining_initial_constructs_summary_mini, aes(x = cell_label, ymin = lower_ci, ymax = upper_ci, color = pseudovirus_inoc), alpha = 0.2, width = 0.2, position = position_dodge(width = 0.6)) +
...
```{r Seeing how the kozaks alter infection, error = FALSE}
kozaks_combined <- infection_data %>% filter(expt == "Kozaks" & pseudovirus_inoc != 0 & recombined_construct != "None" & date != "200706")
kozaks_combined$pseudovirus_env = factor(kozaks_combined$pseudovirus_env, levels = c("VSVG","SARS1","SARS2"))

kozaks_combined_wts <- kozaks_combined %>% filter(cell_label %in% c("ACE2(dEcto)","ACE2","ACE2(low)"))
...
kozaks_combined_suboptimal <- kozaks_combined %>% filter(cell_label %in% c("ACE2(low)","ACE2(dEcto)","ACE2(low)-K31D","ACE2(low)-K353D"))
##kozaks_combined_suboptimal <- merge(kozaks_combined_suboptimal, recombined_construct_key, by = "cell_label")  ## No longer necessary

for(x in 1:nrow(kozaks_combined_suboptimal)){kozaks_combined_suboptimal$scaled_infection[x] <- kozaks_combined_suboptimal$moi[x]/(kozaks_combined_suboptimal[kozaks_combined_suboptimal$recombined_construct == "G755A" & kozaks_combined_suboptimal$pseudovirus_env ==  kozaks_combined_suboptimal$pseudovirus_env[x] & kozaks_combined_suboptimal$pseudovirus_inoc ==  kozaks_combined_suboptimal$pseudovirus_inoc[x] & kozaks_combined_suboptimal$date ==  kozaks_combined_suboptimal$date[x],"moi"])}
...
#kozaks_combined_suboptimal2 <- merge(kozaks_combined_suboptimal2, recombined_construct_key, all.x = T)
...
ggsave(file = "Plots/Kozak_infectivity_plot_suboptimal.pdf", Kozak_infectivity_plot_suboptimal, height = 1.96, width = 1.8)
```
...
for(x in 1:nrow(full_variant_panel)){
  full_variant_panel$scaled_infection[x] <- full_variant_panel$moi[x]/(full_variant_panel[full_variant_panel$recombined_construct == "G755A" & full_variant_panel$pseudovirus_env == full_variant_panel$pseudovirus_env[x] & full_variant_panel$pseudovirus_inoc == full_variant_panel$pseudovirus_inoc[x] & full_variant_panel$date ==  full_variant_panel$date[x],"moi"])}
...
Full_variant_panel_plot <- ggplot() + theme_bw() + 
  ...
  #scale_color_manual(values = virus_colors) +
  scale_y_log10(limits = c(0.01,3)) + labs(x = NULL, y = "Fold infection") +   #scale_y_log10(limits = c(0.035,4))
  geom_hline(yintercept = 1, linetype = 2, alpha = 0.5) +
  ...
  #geom_point(data = full_variant_panel2, aes(x = cell_label, y = default_low_mean, color = pseudovirus_env), position = position_dodge(width = 0.4), size = 2, shape = 8)
...
full_variant_panel3 <- full_variant_panel2 %>% filter(!is.na(variant) & pseudovirus_env == "SARS2")

procko_muts2 <- merge(full_variant_panel3, procko_muts, by = "variant", all = T)


planned_muts <- read.csv(file = "Data/Planned_muts.csv", header = T, stringsAsFactors = F)
procko_muts3 <- merge(procko_muts2, planned_muts, by = "variant", all = T)

procko_muts3$procko_mean <- (procko_muts3$high + -procko_muts3$low)/2

procko_muts3b <- rbind(procko_muts3[1,],procko_muts3)
procko_muts3b[1,"geo_mean"] <- 1; procko_muts3b[1,"mean"] <- 1; procko_muts3b[1,"variant"] <- "WT"

## rescaling for easier interpretation
procko_muts3b$procko_rescaled <- 2^procko_muts3b$procko_mean

ggplot() + 
  theme_bw() + 
  scale_x_log10() + 
  geom_point(data = procko_muts3b, aes(x = mean, y = -low)) + 
  geom_text_repel(data = procko_muts3b, aes(x = mean, y = -low, label = variant))

ggplot() + theme_bw() + 
  scale_x_log10() + 
  geom_point(data = procko_muts3b, aes(x = mean, y = high)) + 
  geom_text_repel(data = procko_muts3b, aes(x = mean, y = high, label = variant))

Monomeric_v_multimeric_ACE2_binding_plot <- ggplot() + theme_bw() + theme(panel.grid.minor = element_blank()) +
  labs(x = "Monomeric binding (Procko)", y = "Oligomeric binding (Infection)") +
  scale_x_log10(limits = c(0.2,5), breaks = c(0.3,1,3)) + scale_y_log10(limits = c(0.1,1.4)) + 
  geom_abline(slope = 1, intercept = 0, alpha = 0.2, size = 5) +
  geom_text_repel(data = procko_muts3b %>% filter(mean != 0), aes(x = procko_rescaled, y = mean, label = variant), color = "red", segment.colour = "orange") +
  geom_text_repel(data = procko_muts3b %>% filter(mean < temp_control_low_mean), aes(x = procko_rescaled, y = default_low_mean, label = variant), color = "red", segment.colour = "orange") +
  geom_point(data = procko_muts3b %>% filter(mean != 0), aes(x = procko_rescaled, y = mean)) + 
  geom_point(data = procko_muts3b %>% filter(mean < temp_control_low_mean), aes(x = procko_rescaled, y = default_low_mean), shape = 8, color = "blue")
Monomeric_v_multimeric_ACE2_binding_plot

procko_density_plot <- ggplot() + theme_bw() + theme(panel.grid.minor = element_blank()) +
  scale_x_log10(limits = c(0.2,5)) + scale_y_continuous(breaks = c(0,1,2)) +
  labs(x = element_blank(), y = "Density") +
  geom_density(data = procko_muts3b, aes(x = procko_rescaled))

ggsave(file = "Plots/Monomeric_v_multimeric_ACE2_binding_plot.pdf", Monomeric_v_multimeric_ACE2_binding_plot, height = 3, width = 3)

patchwork <- procko_density_plot / Monomeric_v_multimeric_ACE2_binding_plot
patchwork + plot_layout(heights = c(1, 9))
```
...
procko_muts3c <- merge(procko_muts3b, gnomad_variant[,c("variant","gnomad_allele_count","gnomad_allele_number","gnomad_frequency")], all.x = T) %>% filter(!is.na(mean) | !is.na(gnomad_allele_count))
procko_muts3c[procko_muts3c$variant == "WT","gnomad_frequency"] <- approximate_wt_gnomad_frequency

procko_muts3c_followup <- procko_muts3c[,c("variant","cell_label","mean","procko_rescaled","gnomad_frequency")] %>% arrange(desc(gnomad_frequency))

ggplot() + theme_bw() + labs(x = "Allele frequency in GnomAD", y = "Oligomeric binding (infection)") +
  scale_x_log10(limits = c(1e-6,1)) + scale_y_log10(limits = c(0.17,1.3)) + 
  geom_text_repel(data = procko_muts3c, aes(x = gnomad_frequency, y = mean, label = variant), color = "red", segment.colour = "orange") +
  geom_point(data = procko_muts3c, aes(x = gnomad_frequency, y = mean))
```