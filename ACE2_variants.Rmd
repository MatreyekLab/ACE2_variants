...
n501y_panel2 <- n501y_panel %>% group_by(virus_label, cell_label) %>% summarize(geo_mean = mean(log_scaled_infection), sd = sd(log_scaled_infection), count = sum(count), cell_label = unique(cell_label), .groups = 'drop')
...
Spike_variant_plot <- ggplot() + theme_bw() + 
  ...
  scale_y_log10(limits = c(0.005,3)) + labs(x = NULL, y = "Fold infection") +   #scale_y_log10(limits = c(0.035,4))
  geom_hline(yintercept = 1, linetype = 2, alpha = 0.5) +
  geom_errorbar(data = n501y_panel2, aes(x = cell_label, ymin = lower_ci, ymax = upper_ci, group = virus_label), alpha = 0.4, width = 0.4, position = position_dodge(width = 0.75)) +
  ...
  geom_point(data = n501y_panel2, aes(x = cell_label, y = mean, color = virus_label), position = position_dodge(width = 0.75), size = 6, shape = 95)
Spike_variant_plot
ggsave(file = "Plots/Spike_variant_plot.pdf", Spike_variant_plot, height = 2.4, width = 1.8*4)
...
n501y_g882a <- n501y_panel2 %>% filter(pseudovirus_env == "G882A") %>% mutate(d614g_mean = mean) %>% select(c("cell_label","d614g_mean"))

n501y_panel3 <- merge(n501y_g742a, n501y_g908a, by = "cell_label")
n501y_panel3 <- merge(n501y_panel3, n501y_g882a, by = "cell_label")

ggplot() + theme_bw() + scale_x_log10() + scale_y_log10() + 
  geom_abline(slope = 1, alpha = 0.2, size = 4) +
  geom_point(data = n501y_panel3, aes(x = wt_mean, y = d614g_mean)) + 
  geom_text_repel(data = n501y_panel3, aes(x = wt_mean, y = d614g_mean, label = cell_label), color = "red", segment.color = "orange", segment.alpha = 0.5)

ggplot() + theme_bw() + scale_x_log10() + scale_y_log10() + 
  geom_abline(slope = 1, alpha = 0.2, size = 4) +
  geom_point(data = n501y_panel3, aes(x = wt_mean, y = n501y_mean)) + 
  geom_text_repel(data = n501y_panel3, aes(x = wt_mean, y = n501y_mean, label = cell_label), color = "red", segment.color = "orange", segment.alpha = 0.5)

ggplot() + theme_bw() + scale_x_log10() + scale_y_log10() + 
  geom_abline(slope = 1, alpha = 0.2, size = 4) +
  geom_point(data = n501y_panel3, aes(x = d614g_mean, y = n501y_mean)) + 
  geom_text_repel(data = n501y_panel3, aes(x = d614g_mean, y = n501y_mean, label = cell_label), color = "red", segment.color = "orange", segment.alpha = 0.5)
```
