...
for(x in 1:nrow(full_variant_t_test)){
  ...
  if(temp_p_value <= full_variant_t_test_sig_threshold){full_variant_t_test$significant[x] <- "yes"}
}
```
...
panel1 <- c("ACE2(dEcto)","ACE2(low)", "ACE2(low)-I21N", "ACE2(low)-D38H", "ACE2(low)-Y41A", "ACE2(low)-Q42R", "ACE2(low)-Y83F", "ACE2(low)-E329K", "ACE2(low)-R357T")
...
Variant_panel1_plot <- ggplot() + theme_bw() + 
  theme(panel.grid.major.x = element_blank(), panel.grid.minor = element_blank(), axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), plot.title = element_text(hjust = 0.5)) + #legend.position = "none"
  scale_color_manual(values = virus_colors) +
  scale_y_log10(limits = c(0.01,3), expand = c(0,0)) + labs(x = NULL, y = "Fold infection") + 
  geom_hline(yintercept = 1, linetype = 2, alpha = 0.5) +
...
ggsave(file = "Plots/Variant_panel1_plot.pdf", Variant_panel1_plot, height = 2*0.95, width = (9*0.5 + 2))
...
panel2 <- c("ACE2(low)-K26R", "ACE2(low)-M82I", "ACE2(low)-E35K", "ACE2(low)-I21V", "ACE2(low)-T27A", "ACE2(low)-G352V", "ACE2(low)-E23K", "ACE2(low)-K26E")
...
ggsave(file = "Plots/Variant_panel2_plot.pdf", Variant_panel2_plot, height = 2*0.95, width = 8*0.5 + 2.02)
...
ggsave(file = "Plots/Li_data_comparison_plot.pdf", Li_data_comparison_plot, height = 2.4, width = 2)
...
procko_muts2$procko_mean <- (procko_muts2$high + -procko_muts2$low)/2
...
procko_muts2b$procko_rescaled_mean <- 2^procko_muts2b$procko_mean

ggplot() + 
  theme_bw() + 
  scale_y_log10() + 
  geom_point(data = procko_muts2b, aes(x = -low, y = mean)) + 
  geom_text_repel(data = procko_muts2b, aes(x = -low, y = mean, label = variant))

ggplot() + theme_bw() + 
  scale_y_log10() + 
  geom_point(data = procko_muts2b, aes(x = high, y = mean)) + 
  geom_text_repel(data = procko_muts2b, aes(x = high, y = mean, label = variant))

procko_muts2b_complete <- procko_muts2b %>% filter(!is.na(procko_rescaled_mean) & !is.na(mean))
...
Monomeric_v_multimeric_ACE2_binding_plot <- ggplot() + theme_bw() + theme(panel.grid.minor = element_blank()) +
  ...
  scale_x_log10(limits = c(0.1,5), breaks = c(0.1,0.3,1,3)) + scale_y_log10(limits = c(0.1,1.4)) + 
  geom_abline(slope = 1, intercept = 0, alpha = 0.2, size = 5) +
  ...
  geom_text_repel(data = procko_muts2b_complete %>% filter(mean < temp_control_low_mean), aes(x = procko_rescaled_mean, y = default_low_mean, label = variant), color = "red", segment.colour = "orange", size = 3) +
  geom_segment(data = procko_muts2b_complete, aes(x = procko_rescaled_mean, xend = procko_rescaled_high, y = mean, yend = mean), color = "cyan", alpha = 0.2) + 
  geom_segment(data = procko_muts2b_complete, aes(x = procko_rescaled_mean, xend = procko_rescaled_low, y = mean, yend = mean), color = "purple", alpha = 0.2) + 
  geom_segment(data = procko_muts2b_complete %>% filter(mean < temp_control_low_mean), aes(x = procko_rescaled_mean, xend = procko_rescaled_high, y = default_low_mean, yend = default_low_mean), color = "cyan", alpha = 0.2) + 
  geom_segment(data = procko_muts2b_complete %>% filter(mean < temp_control_low_mean), aes(x = procko_rescaled_mean, xend = procko_rescaled_low, y = default_low_mean, yend = default_low_mean), color = "purple", alpha = 0.5, alpha = 0.2) + 
  geom_point(data = procko_muts2b_complete, aes(x = procko_rescaled_mean, y = mean)) + 
  geom_point(data = procko_muts2b_complete, aes(x = procko_rescaled_low, y = mean), size = 0.5, color = "purple", alpha = 0.5) + 
  geom_point(data = procko_muts2b_complete, aes(x = procko_rescaled_high, y = mean), size = 0.5, color = "cyan") +
  geom_point(data = procko_muts2b_complete %>% filter(mean < temp_control_low_mean), aes(x = procko_rescaled_low, y = default_low_mean), size = 0.5, color = "purple", alpha = 0.2) + 
  geom_point(data = procko_muts2b_complete %>% filter(mean < temp_control_low_mean), aes(x = procko_rescaled_high, y = default_low_mean), size = 0.5, color = "cyan", alpha = 0.2) +
  geom_point(data = procko_muts2b_complete %>% filter(mean < temp_control_low_mean), aes(x = procko_rescaled_mean, y = default_low_mean), shape = 1, color = "black")
...
procko_density_plot <- ggplot() + theme_bw() + theme(panel.grid.minor = element_blank()) +
  ...
  geom_density(data = procko_muts2b, aes(x = procko_rescaled_mean))

ggsave(file = "Plots/Soluble_RBD_plot.pdf", Monomeric_v_multimeric_ACE2_binding_plot, height = 2.4, width = 3)

## Report out some values for the manuscript
paste("n = ", nrow(procko_muts2b_complete))

paste("The Pearson's R^2 is", round(cor(procko_muts2b_complete$procko_rescaled_mean, procko_muts2b_complete$mean, method = "pearson", use = "complete.obs")^2,2))

paste("The Pearson's p value is", round(cor.test(procko_muts2b_complete$procko_rescaled_mean,procko_muts2b_complete$mean, method = "pearson", use = "complete.obs")$p.value,2))

paste("The Spearman's R^2 is", round(cor(procko_muts2b_complete$procko_rescaled_mean, procko_muts2b_complete$mean, method = "spearman", use = "complete.obs")^2,2))
```
